...
void stampa_tutti_lavoratori(riga dati[], int num_dati)
{
    contatori i, j, k;
    char **id_totali;
    traccia_dati indice_entrata, indice_uscita, turno, num_id;
    ...
    while (esci_while == False)
    {
        ...
        // controllo per uscire dal while + eventuale liberazione memoria allocata a matrice degli id
        k++;
        if (k == num_id)
        {
            esci_while = True;
...
void stampa_lavoratore(char *id_scelto, riga dati[], int num_dati)
{
    contatori i;
    traccia_dati indice_entrata, indice_uscita, turno, num_id;
    bool esci_while, controllo;
...
int main()
{
    ...
    if (strcmp(id_scelto, "0") == 0)
        stampa_tutti_lavoratori(dati, num_dati);
    else
        stampa_lavoratore(id_scelto, dati, num_dati);

    // liberazione memoria allocata e chiusura file
    free(dati);
    fclose(file_handle);
